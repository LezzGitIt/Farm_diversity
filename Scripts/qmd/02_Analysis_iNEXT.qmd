...
::: callout-note
**Reference document — not part of the reproducible pipeline.** The runnable
version of the estimation is `Scripts/00_bird_diversity_estimates.R` (data prep →
`iNEXT4steps` → the two `Tax_div_*` CSVs, self-contained). This `.qmd` is kept for
the interpretation: the Cmax rationale, the iNEXT 4-steps framework, and the
sample-completeness / rarefaction / evenness diagnostic plots.
:::
...
Site_join <- Site_covs %>% select(Id_muestreo_no_dc, Id_gcs, Habitat)
Event_join <- Event_covs %>% select(
  Id_muestreo_no_dc, Id_muestreo, Uniq_db, Fecha, Pc_start, Ano_grp, Season, Rep_ano_grp
  )

# Join together
Event_covs2 <- Site_join %>% left_join(Event_join) %>% 
...
```{r}
#| label: rm-data-low-sc

## External SNAPP folder holding the no-removal run whose SC_obs values flag the low-coverage assemblages; only the second pass (first_run == FALSE) needs it
path_td <- "/Users/aaronskinner/Library/CloudStorage/OneDrive-UBC/Academia/PhD/SCR_SNAPP/SCR My products/Taxonomic_diversity/Excels/"

if(first_run == FALSE){
  if(any(str_detect(Row_id_inext, "Id_gcs"))){
    results_no_rm <- read_excel(path = paste0(path_td, "Tax_div_Rd3_no_rm_06.03.26.xlsx"))
  }
  if(any(str_detect(Row_id_inext, "Id_muestreo"))){
    results_no_rm <- read_excel(path = paste0(path_td, "Tax_div_Rd3_point_count_no_rm.xlsx"), sheet = "Tax_div")
  }
}
```
...
## ENSURE JUST SUBSETTING PROBLEMATIC YEARS AND DATA COLLECTORS
Rm_farms <- results_no_rm %>% 
  arrange(SC_obs) %>% 
  filter(SC_obs < .65 & Order.q == 0) %>%
...
Bird_pcs_analysis2 <- Bird_pcs_analysis2 %>% 
  anti_join(Rm_farms)
}
```
...
if(first_run == FALSE){
  Rm_farms %>% arrange(across(all_of(Row_id_inext)))
}
```

Note that while this is 8 assemblages, there is only one farm that is actually removed (Cipav 1262), b/c the other farms are all sampled in other periods.
...
Create a custom function to sum the number of individuals appropriately, and pivot data frame so each species is a row & each column is a unique combination of `r Row_id_inext`.
...
pivot_wider_inext <- function(row_id, level.farm = FALSE){
  if(level.farm){
    Bird_pcs_analysis3 <- Bird_pcs_analysis3 %>% filter(!is.na(Id_gcs))
  }
  Bird_pcs_analysis3 %>%
    mutate(Identifier = pmap_chr(
...
length(unique(Bird_pcs_analysis3$Species_ayerbe))
```
...
Mult_dc_farms <- Bird_pcs_analysis3 %>% 
  count(Uniq_db, Id_gcs) %>%
  count(Id_gcs, sort = T) %>% 
  filter(n > 1)
```
...
Num.hab.df <- Event_covs2 %>%
  distinct(Identifier, Id_gcs, Uniq_db, Ano_grp, Id_muestreo, Season, Habitat) %>% 
  filter(!is.na(Id_gcs)) %>%
  mutate(Num.hab = as.factor(length(unique(Habitat))), 
         .by = Identifier) %>% 
  arrange(desc(Num.hab))
Num.hab.df2 <- Num.hab.df %>% distinct(Identifier, Num.hab)

## Can explore the combos of habitats that appear together 
# Num.hab == 1 is always forest EXCEPT a single CIPAV farm where Habitat == "Ssp"
# Num.hab == 2 is always either SSP & Pasture or SSP & Forest
Num.hab.df %>%
  distinct(Num.hab, Habitat) %>% # Identifier, 
  #pivot_wider(id_cols = c(Identifier, Num.hab))
  arrange(Num.hab)
```

# iNEXT 4 Steps

Now that the data is formatted, we can follow the iNEXT 4 steps workflow outlined in Chao et al 2020. We bootstrap the analysis 500 times to obtain estimates of uncertainty as well (raised from 100 so the per-assemblage standard errors are stable enough to use quantitatively downstream, e.g. as measurement error in `Scripts/03_linking_model.R`).

```{r}
#| label: iNEXT-4steps
#| cache: true

i4steps <- iNEXT4steps(data = bw_df_farm, nboot = 500, details = TRUE, datatype = "abundance")
#warnings()
```
...
Join the summary dataframes together with dfs_long data frames & Num.hab.df2
...
summary_df <- i4steps$summary[[2]]  %>% 
  ...
  left_join(Num.hab.df2, by = join_by("Assemblage" == "Identifier"))

# Separate Assemblage column back into component parts 
...
summary_join <- summary_df2[,c("Assemblage", "Id_gcs", "Uniq_db", "Ano_grp")] %>%
    distinct()

i4_meta <- map(i4_flat, \(df){ 
  df %>% left_join(summary_join) %>% 
    left_join(Num.hab.df2[,c("Identifier", "Num.hab")], 
...
Export @lst-summary-df (i.e. summary_df2) for sharing with collaborators

```{r}
#| label: export-database

## First pass only: the full assemblage set (used downstream for the q = 1 / q = 2 asymptotic estimates)
# Repo-local only -- the previous external-SNAPP xlsx write was dropped so a run has no side effects outside this repo
if(first_run){
  summary_df2 %>% write_csv(paste0("Derived/Excels/Tax_div_all_farms_", format(Sys.Date(), "%m.%d.%y"), ".csv"))
}
```
...
# Export dataset with higher sample coverage
if(!first_run){
  summary_df2 %>% write_csv(paste0("Derived/Excels/Tax_div_coverage65_", format(Sys.Date(), "%m.%d.%y"), ".csv"))
}
```
