...
### Scale-of-effect preparation. An assemblage -- [data collector . farm . year-group . season] -- is the unit the iNEXT diversity estimates are computed at. For each assemblage this takes the convex hull of the point counts that assemblage sampled, buffers that hull outward at 18 radii (200 m to 2000 m in 200 m steps, then 3000 m to 10 km in 1 km steps), and extracts the mean woody-vegetation canopy cover inside each buffered hull from the WVCC raster year matching the assemblage's survey year. Every assemblage has exactly one Id_gcs and one survey year, so the output feeds Scripts/02b_scale_of_effect.R directly with no farm / group roll-up.
